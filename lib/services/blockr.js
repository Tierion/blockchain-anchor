...
module.exports = {
  getUnspentOutputsAsync: async (address, serviceOptions) => {
    ...
    try {
      ...
      if (error.statusCode) throw new Error(`Invalid response : ${error.statusCode} : ${JSON.parse(error.error).message}`)
  ...
  confirmEthDataAsync: async (transactionId, expectedValue, serviceOptions) => {
    throw new Error('Eth functions not supported in blocker service')
  }
}
