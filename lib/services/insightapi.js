...
module.exports = {
  getUnspentOutputsAsync: async (address, serviceOptions) => {
    let targetUrl = `${serviceOptions.insightApiOrigin}/api/addr/${address}/utxo?noCache=1`
  ...
  pushTransactionAsync: async (transactionHex, serviceOptions) => {
    let options = {
      method: 'POST',
      url: `${serviceOptions.insightApiOrigin}/api/tx/send`,
  ...
  confirmOpReturnAsync: async (transactionId, expectedValue, serviceOptions) => {
    let targetUrl = `${serviceOptions.insightApiOrigin}/api/tx/${transactionId}`
  ...
  confirmBTCBlockHeaderAsync: async (blockHeight, expectedValue, serviceOptions) => {
    let targetUrl = `${serviceOptions.insightApiOrigin}/api/block-index/${blockHeight}`
    ...
    targetUrl = `${serviceOptions.insightApiOrigin}/api/block/${apiResult.blockHash}`
    options = {
      method: 'GET',
      url: targetUrl,
      resolveWithFullResponse: true
